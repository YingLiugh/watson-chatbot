...
function updateMessage(res, input, response) {
  if (!response.output) {
    ...
    var path = getLocationURL(response.context.city);
    var query = "http://api.wunderground.com" + path;
    axios
      .get(query)
      .then(resp => {
        var params = [];
        console.log(resp.data.current_observation.weather);
        params.push(resp.data.current_observation.weather);
        params.push(resp.data.current_observation.relative_humidity);
        console.log(params);
      ...
      .get(options, function(resp) {
        ...
        resp.on("end", function() {
          ...
          if (chunkJSON.location) {
            ...
            //day after tomorrow
            params.push([]
              chunkJSON.forecast.txt_forecast.forecastday[when * 2].fcttext
            );
        ...
        if (row.doc) {
          ...
          if (doc.response) {
            ...
            if (doc.response.entities && doc.response.entities.length > 0) {
              entity =
                doc.response.entities[0].entity +
                " : " +
